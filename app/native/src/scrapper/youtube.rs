...
impl YoutubeInfo {
    pub fn new(mut url: &str)
    {
        &self.
    }
    
    fn format_url (mut url: &str) -> String
    {
        ...
        if url_regex.is_match(url) 
        {
            ...
            url = vid_split.get(1).unwrap().as_str();
        }

        return format!("{}/get_video_info?video_id=${}", YOUTUBE_URL, url);
    }
}
    ...
    pub fn video_info_url(vid: &str) -> String {
        let vid = percent_encoding::utf8_percent_encode(vid, percent_encoding::NON_ALPHANUMERIC).to_string();
